...
static void one_one_do_benchmarks(struct ng_module *module) {
  ...
  char *buffer, *buffer2; // = (char*)module->malloc(max_data_size);
  ...
  for (int i = 0; i < max_data_size; i++){ buffer[i] = 0xff; buffer2[i] = 0xff;}
  ...
  for (data_size = g_options.min_datasize; data_size > 0;
    ...
    if (data_size % g_options.concurrent || data_size < g_options.concurrent) {
      ng_error("Data size must be divisible for and larger than --concurrent.");
      ng_exit(10);
    }
...
#else
    if(g_options.concurrent > 1 || p > 2){
      fprintf(stderr, "Concurrent flag and p > 2 can only be used with MPI.\n");
      exit(-1);
    }
#endif
    ...
    for (int test = -1 /* 1 warmup test */; test < test_count; test++) {
      ...
      if(p > 2){
	    ...
	    if (rank % 2 == 1) {
          ...
          if(g_options.concurrent > 1){
#ifdef NG_MPI            
            for(size_t conc = 0; conc < g_options.concurrent; conc++){
              MPI_Irecv(buffer + conc*conc_chunk_size, conc_chunk_size, MPI_BYTE, g_options.mpi_opts->partner, 0, MPI_COMM_WORLD, &reqs[conc]);
            }

            MPI_Waitall(g_options.concurrent, reqs, MPI_STATUSES_IGNORE);
	  
            for(size_t conc = 0; conc < g_options.concurrent; conc++){
              MPI_Isend(buffer + conc*conc_chunk_size, conc_chunk_size, MPI_BYTE, g_options.mpi_opts->partner, 0, MPI_COMM_WORLD, &reqs_2[conc]);
	          }	    
#endif
          }else{
...
#ifdef NG_MPI            
            for(size_t conc = 0; conc < g_options.concurrent; conc++){
              MPI_Isend(buffer + conc*conc_chunk_size, conc_chunk_size, MPI_BYTE, g_options.mpi_opts->partner, 0, MPI_COMM_WORLD, &reqs[conc]);
              MPI_Irecv(buffer2 + conc*conc_chunk_size, conc_chunk_size, MPI_BYTE, g_options.mpi_opts->partner, 0, MPI_COMM_WORLD, &reqs_2[conc]);
            }

            HRT_GET_TIMESTAMP(t[1]);
            MPI_Waitall(g_options.concurrent, reqs_2, MPI_STATUSES_IGNORE);
#endif
          }else{            
            NG_SEND(g_options.mpi_opts->partner, buffer, data_size, module);

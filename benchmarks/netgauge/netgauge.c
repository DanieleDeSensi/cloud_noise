...
int ng_get_options(int *argc, char ***argv, struct ng_options *options) {
  ...
  /* concurrent */
  options->concurrent = args_info.concurrent_arg;
  /* gradation */
...
FILE *open_output_file(char *filename) {
	...
	if (fd == NULL) {
                ...
                if (uname (&uninfo) < 0) {
                     ng_error("Could not open output file %s. \n",filename);
                }else{
                     ng_error("Could not open output file %s. on node %s \n",filename,uninfo.nodename);
                }

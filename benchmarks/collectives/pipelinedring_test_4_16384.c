...
int main(int argc, char** argv){
    ...
    for(i = -10; i < 10000; i++){
        ...
        MPI_Send(buffer, 4096, MPI_BYTE, 1, 0, MPI_COMM_WORLD);
        do{
            if(completed[1] == 0){
                MPI_Test(&(mpi_reqs[1]), &(flags[1]), MPI_STATUS_IGNORE);
                if(flags[1] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 1, 1, MPI_COMM_WORLD);
                    completed[1] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[3] == 0){
                ...
                if(flags[3] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 1, 2, MPI_COMM_WORLD);
                    completed[3] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[5] == 0){
                ...
                if(flags[5] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 1, 3, MPI_COMM_WORLD);
                    completed[5] = 1;
                    completed_tot += 1;
                }
            }
        ...
        MPI_Send(buffer, 4096, MPI_BYTE, 2, 0, MPI_COMM_WORLD);
        do{
            if(completed[1] == 0){
                MPI_Test(&(mpi_reqs[1]), &(flags[1]), MPI_STATUS_IGNORE);
                if(flags[1] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 2, 1, MPI_COMM_WORLD);
                    completed[1] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[3] == 0){
                MPI_Test(&(mpi_reqs[3]), &(flags[3]), MPI_STATUS_IGNORE);
                if(flags[3] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 2, 2, MPI_COMM_WORLD);
                    completed[3] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[5] == 0){
                MPI_Test(&(mpi_reqs[5]), &(flags[5]), MPI_STATUS_IGNORE);
                if(flags[5] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 2, 3, MPI_COMM_WORLD);
                    completed[5] = 1;
                    completed_tot += 1;
                }
            }
        ...
        MPI_Send(buffer, 4096, MPI_BYTE, 3, 0, MPI_COMM_WORLD);
        do{
            if(completed[1] == 0){
                MPI_Test(&(mpi_reqs[1]), &(flags[1]), MPI_STATUS_IGNORE);
                if(flags[1] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 3, 1, MPI_COMM_WORLD);
                    completed[1] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[3] == 0){
                MPI_Test(&(mpi_reqs[3]), &(flags[3]), MPI_STATUS_IGNORE);
                if(flags[3] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 3, 2, MPI_COMM_WORLD);
                    completed[3] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[5] == 0){
                MPI_Test(&(mpi_reqs[5]), &(flags[5]), MPI_STATUS_IGNORE);
                if(flags[5] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 3, 3, MPI_COMM_WORLD);
                    completed[5] = 1;
                    completed_tot += 1;
                }
            }
        ...
        MPI_Send(buffer, 4096, MPI_BYTE, 0, 0, MPI_COMM_WORLD);
        do{
            if(completed[1] == 0){
                MPI_Test(&(mpi_reqs[1]), &(flags[1]), MPI_STATUS_IGNORE);
                if(flags[1] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 0, 1, MPI_COMM_WORLD);
                    completed[1] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[3] == 0){
                MPI_Test(&(mpi_reqs[3]), &(flags[3]), MPI_STATUS_IGNORE);
                if(flags[3] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 0, 2, MPI_COMM_WORLD);
                    completed[3] = 1;
                    completed_tot += 1;
                }
            }
            if(completed[5] == 0){
                MPI_Test(&(mpi_reqs[5]), &(flags[5]), MPI_STATUS_IGNORE);
                if(flags[5] == 1){
                    MPI_Send(buffer, 4096, MPI_BYTE, 0, 3, MPI_COMM_WORLD);
                    completed[5] = 1;
                    completed_tot += 1;
                }
            }

...
typedef struct
{
  ...
  btime_t starttime, Gval; // only for visualization
...
btime_t getRandElement(std::vector<btime_t> &pvec)
{
  int pos = uint_dist(rng) % pvec.size();
  //int pos = std::rand() % pvec.size();
  //std::cout << pos << "/" << pvec.size() << " " << pvec[pos] << std::endl;
  return pvec[pos];
}
...
int main(int argc, char **argv)
{
  ...
  // const int G=args_info.LogGOPS_G_arg;
  std::vector<btime_t> Lv;
  std::vector<btime_t> Lv_far;
  std::vector<btime_t> Gv;
  std::vector<btime_t> Gv_far;

  if (args_info.LogGOPS_Lfile_given)
  {
    std::ifstream fin(args_info.LogGOPS_Lfile_arg);
    btime_t Lread;
    while (fin >> Lread)
      Lv.push_back(Lread);
    fin.close();
  ...
  {
    Lv.push_back(args_info.LogGOPS_L_arg);
  }

  if (args_info.LogGOPS_Lfile_far_given)
  {
    std::ifstream fin(args_info.LogGOPS_Lfile_far_arg);
    btime_t Lread;
    while (fin >> Lread)
      Lv_far.push_back(Lread);
    fin.close();
    //auto first = Lv_far.begin() + 7000;
    //auto last = Lv_far.begin() + 8000;
    //Lv_far = std::vector<btime_t>(first, last);
  }
  else
  {
    Lv_far = Lv;
  ...
  if (args_info.LogGOPS_Gfile_given)
  {
    std::ifstream fin(args_info.LogGOPS_Gfile_arg);
    btime_t Gread;
    while (fin >> Gread)
      Gv.push_back(Gread);
    fin.close();
  ...
  {
    Gv.push_back(args_info.LogGOPS_G_arg);
  }

  if (args_info.LogGOPS_Gfile_far_given)
  {
    std::ifstream fin(args_info.LogGOPS_Gfile_far_arg);
    btime_t Gread;
    while (fin >> Gread)
      Gv_far.push_back(Gread);
    fin.close();
  }
  else
  {
  ...
  {
    rng.seed(args_info.seed_arg);
    //std::srand(args_info.seed_arg);
  }
  ...
  printf("size: %i (%i CPUs, %i NICs); L=%i (num Ls: %lu), o=%i g=%i, G=%i (num Gs: %lu), O=%i, P=%i, S=%u\n",
         p, ncpus, nnics, Lv[0], Lv.size(), o, g, Gv[0], Gv.size(), O, p, S);
  ...
  for (Parser::schedules_t::iterator sched = parser.schedules.begin(); sched != parser.schedules.end(); ++sched, ++host)
  {
    ...
    for (SerializedGraph::nodelist_t::iterator freeop = free_ops.begin(); freeop != free_ops.end(); ++freeop)
    {
      ...
      case OP_RECV:
        if (print)
          printf("init %i (%i,%i) recvs from: %i, tag: %i, size: %lu\n", host, freeop->proc, freeop->nic, freeop->target, freeop->tag, (long unsigned int)freeop->size);
        break;
      default:
  ...
  while (!aq.empty() || new_events)
  {
    ...
    case OP_LOCOP:
    {
      ...
      {
        if (print)
          printf("-- locop local o not available -- reinserting\n");
    ...
    { // a send op
      ...
      if (std::max(nexto[elem.host][elem.proc], nextgs[elem.host][elem.nic]) <= elem.time)
      { // local o,g available!
        if (print)
        ...
        int switch_source = elem.host / int(args_info.nodes_per_switch_arg);
        int switch_dest = elem.target / int(args_info.nodes_per_switch_arg);
        if(switch_source == switch_dest){
          Gval = getRandElement(Gv);
          Lval = getRandElement(Lv);
        }else{
          Lval = getRandElement(Lv_far);
          //std::cout << elem.host << " to " << elem.target << " extracted " << Lval << std::endl;
          Gval = getRandElement(Gv_far);
        }

        //std::cout << "Latency " << Lval << std::endl;
        btime_t tmp = elem.size - 1;
        //btime_t Gsum = tmp*Gval;
        btime_t Gsum = 0;
        while(tmp > 0){
          btime_t next_block = std::min((uint64_t) 16777216, tmp);
          if(switch_source == switch_dest){
            Gsum += next_block * getRandElement(Gv);
          }else{
            Gsum += next_block * getRandElement(Gv_far);
          }
          tmp -= next_block;
        }
        ...
        nextgs[elem.host][elem.nic] = elem.time + g + Gsum; // TODO: G should be charged in network layer only
        ...
        elem.time = elem.time + o + Lval; // arrival time of first byte only (G will be charged at receiver ... +(elem.size-1)*G; // arrival time
        if (elem.size > S){
          elem.time += Lval*2;
        }
        ...
        { // eager message
          if (print)
            printf("-- [%i] eager -- satisfy local requires at t: %lu\n", h, (long unsigned int)elem.starttime + o);
    ...
    case OP_RECV:
    {
      ...
      { // found it in local UQ
        ...
        { // rendezvous - free remote request
          ...
          if (print)
            printf("-- satisfy remote requires at host %i (offset: %i)\n", elem.target, elem.offset);
          tlviz.add_transmission(elem.host, matched_elem.src, matched_elem.starttime + o, elem.time, elem.size, matched_elem.Gval);
        }
    ...
    case OP_MSG:
    {
          ...
          std::max(nexto[elem.host][elem.proc], nextgr[elem.host][elem.nic]) <= elem.time /* local o,g available! */)
      {
        ...
        // check if OS Noise occurred
        btime_t noise = osnoise.get_noise(elem.host, elem.time, elem.time + o);
        //nexto[elem.host][elem.proc] = elem.time + o + noise + std::max((elem.size - 1) * O, (elem.size - 1) * elem.Gval) /* message is only received after G is charged !! TODO: consuming o seems a bit odd in the LogGP model but well in practice */;
        //nextgr[elem.host][elem.nic] = elem.time + g + (elem.size - 1) * elem.Gval;
        nexto[elem.host][elem.proc] = elem.time + o + noise + std::max((elem.size - 1) * O, elem.Gsum) /* message is only received after G is charged !! TODO: consuming o seems a bit odd in the LogGP model but well in practice */;
        nextgr[elem.host][elem.nic] = elem.time + g + elem.Gsum;
        ...
        { // found it in RQ
          ...
          { // rendezvous - free remote request
            ...
            if (print)
              printf("-- satisfy remote requires on host %i\n", elem.target);
          }
          tlviz.add_transmission(elem.target, elem.host, elem.starttime + o, elem.time, elem.size, elem.Gval);
          tlviz.add_orecv(elem.host, elem.time + (elem.size - 1) * elem.Gval - (elem.size - 1) * O, elem.time + o + std::max((elem.size - 1) * O, (elem.size - 1) * elem.Gval), elem.proc);
          tlviz.add_noise(elem.host, elem.time + o + std::max((elem.size - 1) * O, (elem.size - 1) * elem.Gval), elem.time + o + noise + std::max((elem.size - 1) * O, (elem.size - 1) * elem.Gval), elem.proc);
          // satisfy local requires
    ...
    if (args_info.progress_given)
    {
      if (num_events / 100 * lastperc < aqtime)
      {
        ...
        for (uint j = 0; j < rq.size(); j++)
          maxrq = std::max(maxrq, (uint)rq[j].size());
        for (uint j = 0; j < uq.size(); j++)
          maxuq = std::max(maxuq, (uint)uq[j].size());
  ...
  if (ok)
  {
    ...
    { // print only maximum
      ...
      std::cout << "Maximum finishing time at host " << host << ": " << max << " (" << (double)max / 1e9 << " ms)\n";
    }

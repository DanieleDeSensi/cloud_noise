...
Goal::t_id Goal::Send(int bufsize, int dest, int tag) {
...
Goal::t_id Goal::Recv(int bufsize, int src, int tag) {
...
void create_binomial_tree_bcast_rank(Goal *goal, int root, int comm_rank, int comm_size, int datasize) {
	...
	Goal::t_id recv = -1, send = -1, last_send = -1;
	for (int r = 0; r < ceil(log2(comm_size)); r++) {
		...
		if ((vrank+pow(2,r) < comm_size) and (vrank < pow(2,r))) {
			...
			if(last_send != -1){
				goal->Requires(send, last_send); // Serialize the send so that they are overlapped between diff ranks
			}
			last_send = send;
		}
...
void create_pipelined_ring(gengetopt_args_info *args_info) {
	...
	for (int comm_rank = 0; comm_rank < comm_size; comm_rank++) {
		...
		for ( int r = 0; r < num_rounds; r++ ) {
			...
			int recvpeer;
			if(vrank == 0){
				recvpeer = comm_size - 1;
			}else{
				recvpeer = vrank - 1;
			} 
			int sendpeer = (vrank + 1) % comm_size;
			...
			if (recvpeer >= 0) {
				recv.at(r) = goal.Recv(psize, recvpeer, r);
			}
			...
			if (sendpeer < comm_size) {
				send.at(r) = goal.Send(psize, sendpeer, r);
			}
			
			// we can not send data before we received it (except for rank 0, someone has to start)
			/*
			...
			*/
			if(r > 0){
				goal.Requires(send.at(r), recv.at(r-1));
			}
		}
		goal.EndRank();
...
/* creates a pingpong algoritm */
void create_pingpong(gengetopt_args_info *args_info) {
	int comm_size = args_info->commsize_arg;
	int datasize = args_info->datasize_arg;
	Goal goal(args_info, comm_size);
  	// each rank
	for (int comm_rank = 0; comm_rank < comm_size; comm_rank++) {
		...
		if(comm_rank % 2 == 0){
			if(comm_rank + 1 < comm_size){
				goal.Send(datasize, comm_rank + 1);
				goal.Recv(datasize, comm_rank + 1);
			}
		}else{
			int r = goal.Recv(datasize, comm_rank - 1);
			int s = goal.Send(datasize, comm_rank - 1);
			goal.Requires(s, r);
		}
	    // beware - this is also stateful and finishes a rank!
		goal.EndRank();
...
int main(int argc, char **argv) {
	...
	if (strcmp(args_info.ptrn_arg, "pingpong") == 0) {
		 create_pingpong(&args_info);
	}

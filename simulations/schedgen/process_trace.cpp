...
static const char *mpifuncs[] = { "MPI_Allreduce" , "MPI_Alltoall", "MPI_Bcast", "MPI_Barrier", "MPI_Init", "MPI_Init_thread", "MPI_Finalize",
...
void change_zero_to_host(char *mask, char *buffer, int host) {
  ...
  int pos = substr-mask+1;
  substr++;
  int len=0;
  while(*substr == '0') { substr++; len++; }
  int exp10len=1;
  for(int i=1; i<len; ++i) exp10len*=10;

  //std::cout << mask << " " << pos << " " << substr << " " << len <<" " << exp10len <<  "\n";
  int strl = strlen(mask);
  assert(strl > pos);
  memcpy(buffer, mask, pos);
  //std::cout << buffer << "\n";
  //sprintf(&buffer[pos], "%i.", host);
  pprint(&buffer[pos], len+1, host, exp10len);
  //std::cout << buffer << "\n";
  int pos2=substr-mask;
  while(buffer[pos2]!='\0') pos2++;
  strcpy(&buffer[pos2], &mask[pos+len]);
  //std::cout << buffer << "\n";
}
...
void process_trace(gengetopt_args_info *args_info) {
  ...
  for(int extrhost = 0; extrhost<extrhosts; ++extrhost) {
    ...
    while(1) {
      ...
      while(!found_eof) {
        // read next line
        found_eof = trcrd.getline(line, 2048);
        lineno++;
        ...
        static const htorMatcher e_init_t(&pars, "MPI_Init_thread");
        if(pars.match(&e_init_t, funchash, line, &match)) {
          double inittime; match.get(6,&inittime);
          tracestart = inittime;
          if(print) std::cout << "starttime host " << host << " = " << tracestart << std::endl;
          assert(curlocop.start == 0);
          curlocop.start = tracestart;

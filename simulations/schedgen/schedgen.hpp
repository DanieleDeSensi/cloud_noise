...
class LocOp {
  ...
  enum t_type { IREQU, REQU }; // type of preceding op (I{send,recv} or {send,recv})
  std::vector<std::pair<int /*Goal::t_id*/,t_type> > prev,next; // preceding and next operations - pairs of id and type
  Goal *goal; // goal object
  double start; // start time for this local operation 

  LocOp(Goal *_goal, double _time_mult, int cpu) : time_mult(_time_mult), cpu(cpu), goal(_goal), start(0)  {}
...
class Goal {
    ...
    typedef std::vector<std::pair<t_id,LocOp::t_type> > locop; /* used to identify local operations for dependencies, it's a vector of pairs of < id , irequ | requ > */
    ...
    void SetTag(int tag) { curtag=tag; }
		...
		int Send(int size, int dest, int tag=-1);
		int Recv(std::vector<buffer_element>, int src);
		int Recv(int size, int src, int tag=-1);
	...
	private:
    std::set<t_id> start, end; /* the operations which are independent at start and end */
		std::string schedule;
		std::string filename;
		std::fstream myfile;
